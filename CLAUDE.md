...
## 絶対禁止事項（コンパクション・再起動後も最優先）

- **計算モデルを独断で決めない** → `docs/spec/01_requirements.md` § 8 の未確定事項は実装前にFP確認（このプロジェクトの最重要制約）
- **実装フローをスキップしない** → planner → tester → architect → engineer の順を守る
